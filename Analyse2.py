...
bar_width = 0.35
plt.bar(x, comparison['Top 5 Avg'], width=bar_width, label='Top 5 Avg', color='blue')
plt.bar(x + bar_width, comparison['Aeronyx'], width=bar_width, label='Aeronyx', color='red')

# Add labels and title
plt.xlabel('Features', fontsize=14)
plt.ylabel('Scores', fontsize=14)
plt.title('Comparison of Scores: Top 5 Avg vs. Aeronyx', fontsize=16)
plt.xticks(x + bar_width / 2, features, rotation=90, fontsize=12)
plt.yticks(fontsize=12)
plt.legend(fontsize=14)

# Annotate scores on the bars
for i in range(len(features)):
    plt.text(i - bar_width/2, comparison['Top 5 Avg'][i] + 0.05, round(comparison['Top 5 Avg'][i], 2), ha='center', fontsize=12)
    plt.text(i + bar_width/2, comparison['Aeronyx'][i] + 0.05, round(comparison['Aeronyx'][i], 2), ha='center', fontsize=12)
